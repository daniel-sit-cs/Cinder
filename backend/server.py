...
class GenerateStoryRequest(BaseModel):
    ...
    frameCount: int = 15
    referenceImage: Optional[str] = None  # base64 encoded PNG
...
# ============================================================================
# IMAGE GENERATION via Replicate (SDXL + IP-Adapter for character consistency)
# ============================================================================
def generate_images_via_replicate(prompt: str, frame_count: int, style: str, ref_image_b64: Optional[str] = None) -> List[tuple]:
    """
    Generates story images using Replicate.
    With a reference image: uses lucataco/ip-adapter-sdxl for character consistency across all frames.
    Without a reference image: falls back to flux-schnell.
    Returns a list of (local_image_path, narration_text) tuples.
    """
    import time
    use_ip_adapter = ref_image_b64 is not None
    print(f"🎨 REPLICATE: Generating {frame_count} images for '{prompt}' (IP-Adapter={'ON' if use_ip_adapter else 'OFF'})")

    beats = STORY_BEATS[:frame_count]
    results = []

    # Encode reference image once for all frames
    ref_image_uri = f"data:image/jpeg;base64,{ref_image_b64}" if use_ip_adapter else None

    for i, (scene_template, narration_template) in enumerate(beats):
        print(f"   ... Frame {i + 1}/{frame_count}")

        scene = scene_template.format(prompt=prompt)
        full_prompt = (
            f"{style} style, highly detailed, cinematic lighting, "
            f"{scene}, featuring {prompt} as the main character"
        )
        narration = narration_template.format(prompt=prompt.split(',')[0])

        if use_ip_adapter:
            # IP-Adapter SDXL Face: conditions every frame on the same reference image → character consistency
            ...
            img_bytes = requests.get(str(img_url)).content
        else:
            # No reference image — use fast flux-schnell
            output = replicate.run(
        ...
        results.append((img_path, narration))
        print(f"   ... Saved frame {i + 1} → {img_filename}")

    return results

# ============================================================================
# VIDEO COMPILER: 15 frames → 1 cinematic .mp4
# ============================================================================
def create_full_story_video(frame_tuples: List[tuple], output_path: str) -> bool:
    """
    Takes a list of (image_path, narration_text) tuples.
    Generates TTS audio for each, applies Ken Burns zoom, concatenates into one video.
    """
    try:
        clips = []
...
async def generate_story(request: GenerateStoryRequest):
    try:
        ...
        frame_tuples = generate_images_via_replicate(
            request.prompt,
            request.frameCount,
            request.style,
